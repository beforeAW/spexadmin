...
const Radio = forwardRef<HTMLInputElement, RadioProps>(
  ({ label, error = false, className = '', ...props }, ref) => {
    const baseStyles =
      'h-4 w-4 border transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 cursor-pointer';
    const normalStyles =
    ...
    if (label) {
      return (
        <label className="flex items-center space-x-2 cursor-pointer">
          <input ref={ref} type="radio" className={combinedClassName} {...props} />
          <span className="text-sm text-karspex-black select-none">{label}</span>
        </label>
      );
    }

    return <input ref={ref} type="radio" className={combinedClassName} {...props} />;
  }
);
